    ...
    <div className="second-footer">
      <div className="icons">
        {array.map((answer, index) => (
            <RenderIcon icon={answer} key={index} />
        ))}
      </div>
      <>
...
function RenderSecondFooter(score) {
    let scoreDeck = score.deck;
...
export default function Footer(card) {
  ...
  if (score < card.deck) {
    return <RenderFooter score={score} deck={card.deck} icon={card.icon} />;
  } else {
    return (
      <>
        <Message status={incorrect} />
        <RenderSecondFooter score={score} deck={card.deck} icon={card.icon} />
